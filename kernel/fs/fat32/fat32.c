 ...
 * @Description: fat32
 * @Author: QIUFUYU
 * @Date: 2021-10-06 20:03:25
 * @LastEditTime: 2021-11-19 20:41:49
 */
...
#include"fs/hdd.h"
#include"qstring.h"
#define PANIC(x) console_debug(x,DEBUG_PANIC);
...
static void getSector(f32 *fs, uint8_t *buff, uint32_t sector, uint32_t count) {
    ...
    ata_read(get_ata_device("/dev/hda"),sector,count,buff);
}

static void putSector(f32 *fs, uint8_t *buff, uint32_t sector, uint32_t count) {
    uint32_t i;
    for(i = 0; i < count; i++) {
        //printf("w1!0x%x\n",get_ata_device("/dev/hda"));
        //ata_pio_write48(sector + i, 1, buff + (i * 512));
        ata_write(get_ata_device("/dev/hda"),sector + i,1,buff + (i * 512));
    }
    //printf("wok\n");
}
...
f32 *makeFilesystem(char *fatSystem) {
    ...
    printf("Filesystem identified! 0x%x\n",fs);
    read_bpb(fs, &fs->bpb);
    //while(1);
...
//根据文件名找到入口
struct dir_entry *get_dir_entry(struct directory *dir,char *name)
{
    if(dir==NULL)
        return NULL;
    if(dir->num_entries==0)
        return NULL;
    //printf("pausen %d %x\n",dir->num_entries,dir->entries[1].name);
    for(int i=0;i<dir->num_entries;i++)
    {
        ...
        //printf("name a:%s , name b:%s\n",dir->entries[i].name,name);
    }
    return NULL;
}
